...
print("\n=== 4.3 SALARIOS POR DEPARTAMENTO (Promedio) ===")
df_depto = modelo[["nombre_departamento", "salario"]].groupby("nombre_departamento").mean().reset_index()
df_depto = df_depto.sort_values("salario", ascending=False).reset_index(drop=True)
df_depto.index = df_depto.index + 1
print(df_depto)

print("\n=== 4.4 PROMEDIO SALARIAL GENERAL ===")
promedio_general = modelo["salario"].mean()
print(f"Promedio salarial general: ${promedio_general:,.2f}")

print("\n=== 5. TOP 10 EMPLEADOS CON MAYOR SALARIO ===")
top10 = modelo.groupby(["id_empleado", "nombre_empleado"])["salario"].mean() \
              .reset_index() \
              .sort_values("salario", ascending=False) \
              .head(10) \
              .reset_index(drop=True)
top10.index = top10.index + 1
top10["salario"] = top10["salario"].map("${:,.2f}".format)
print(top10[["nombre_empleado", "salario"]])